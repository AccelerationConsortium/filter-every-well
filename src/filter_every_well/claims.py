...
class ClaimStore:
    ...
    Must be instantiated *inside* an async context (e.g. inside the
    FastAPI lifespan handler) so the ``asyncio.Lock`` binds to the
    running event loop.
    """
    ...
    async def release(self, token: str | None) -> None:
        """Drop the active claim.  Idempotent."""
        async with self._lock:
            now = _utcnow()
            self._expire_if_due(now)
            ...
            logger.info("claim released by %s/%s", self._owner, self._session_id)
            self._clear_locked()

    async def validate(self, token: str | None) -> bool:
        """Return ``True`` iff ``token`` matches the live claim."""
        async with self._lock:
            now = _utcnow()
            self._expire_if_due(now)
    ...
    async def force_clear(self) -> None:
        """Drop any active claim without requiring a token.  For shutdown
        only — DO NOT call from a request handler."""
        async with self._lock:
    ...
    def _expire_if_due(self, now: datetime) -> None:
        ...
        if now >= self._expires_at:
            logger.info(
                "claim expired (was held by %s/%s)", self._owner, self._session_id
            )
            self._clear_locked()
